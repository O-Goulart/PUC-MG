...
void countingsort(Restaurante *array, int n) {
    ...
    int max_capacidade = array[0].capacidade;
    for (int i = 1; i < n; i++) {
        comparacoes++;
        if (array[i].capacidade > max_capacidade) {
            max_capacidade = array[i].capacidade;
        }
    }

    int *count = (int *)calloc(max_capacidade + 1, sizeof(int));
    ...
    for (int i = 1; i <= max_capacidade; i++) {
        count[i] += count[i - 1];
    }
...
int main() {
    ...
    countingsort(selecionados, num_selecionados);
    ...
    FILE *log = fopen("855217_countingsort.txt", "w");
    if (log != NULL) {
