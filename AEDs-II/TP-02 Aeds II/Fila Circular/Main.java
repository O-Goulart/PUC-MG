...
class FilaCircular {
    ...
    public void inserir(Restaurante dinner) throws Exception {
        ...
        for (int i = primeiro; i != ultimo; i = (i + 1) % array.length) {
            soma += array[i].getData_abertura().getAno();
            count++;
        }
        int media = Math.round((float) soma / count);
                    ...
                    for (int i = 0; i < numOperacoes; i++) {
                        ...
                        try {
                            if (comando.equals("I")) {
                                ...
                                if (dinner != null) fila.inserir(dinner);
