...
public class Main {
    ...
    public static void insercao(Restaurante[] array, int n) {
        for (int i = 1; i < n; i++) {
            ...
            movimentacoes++;
            int j = i - 1;
            // O while do github: while ((j >= 0) && (array[j] > tmp))
            while (j >= 0) {
                comparacoes++; 
    ...
    public static void main(String[] args) {
        ...
        double tempoExecucao = (fimTempo - inicioTempo) / 1000.0; // Converte ms para segundos
        ...
        try {
            FileWriter log = new FileWriter("855217_insercao.txt");
            // O PDF exige que seja separado por tabulação \t
