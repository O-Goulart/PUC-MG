...
public class Main {
    ...
    public static boolean pesquisaSequencial(Restaurante[] array, int n, String nomeBuscado) {
        for (int i = 0; i < n; i++) {
            comparacoes++; 
            if (array[i].getNome().equals(nomeBuscado)) {
                return true;
            }
        }
        return false; 
    ...
    public static void main(String[] args) {
        ...
        if (sc.hasNextLine()) {
            ...
            while (!entrada.equals("FIM")) {
                boolean achou = pesquisaSequencial(selecionados, numSelecionados, entrada);
            ...
            double tempoExecucao = (fimTempo - inicioTempo) / 1000.0;

            try {
                FileWriter log = new FileWriter("855217_sequencial.txt");
                log.write("855217\t" + comparacoes + "\t" + tempoExecucao + "\n");
                log.close();
            } catch (IOException e) {
