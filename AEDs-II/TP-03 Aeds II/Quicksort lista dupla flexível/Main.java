...
public class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            PrintWriter log = new PrintWriter(new FileWriter("855217_quicksort_flexivel.txt"));
            log.printf("855217\t%d\t%d\t%dms\n", numComparacoes, numMovimentacoes, (tempoFinal - tempoInicial));
            log.close();
...
class ListaDupla {
    ...
    public void quicksort() {
        if (primeiro != ultimo && tamanho > 1) {
            quicksortRec(primeiro.prox, 0, ultimo, tamanho - 1);
        }
    }

    private void quicksortRec(CelulaDupla esq, int esqIdx, CelulaDupla dir, int dirIdx) {
        if (esqIdx >= dirIdx) return; 
        ...
        int pivoIdx = (esqIdx + dirIdx) / 2;
        CelulaDupla pNode = esq;
        for (int k = esqIdx; k < pivoIdx; k++) {
            pNode = pNode.prox;
        }
        Restaurante pivo = pNode.elemento;

        while (iIdx <= jIdx) {
            while (compare(i.elemento, pivo) < 0) {
                i = i.prox;
                iIdx++;
            }
            while (compare(j.elemento, pivo) > 0) {
                j = j.ant;
                jIdx--;
            }
        ...
        // Chamadas recursivas com índices para garantir que nao ocorrerá StackOverflow
        if (esqIdx < jIdx) quicksortRec(esq, esqIdx, j, jIdx);
        if (iIdx < dirIdx) quicksortRec(i, iIdx, dir, dirIdx);
    }

    private int compare(Restaurante a, Restaurante b) {
        Main.numComparacoes++;
        int cmp = Double.compare(a.getAvaliacao(), b.getAvaliacao());
        if (cmp == 0) {
            Main.numComparacoes++;
