...
public class Main {
    ...
    // Método auxiliar para comparar dois restaurantes pela Avaliação e Desempate por Nome
    ...
    public static void quicksortParcial(Restaurante[] array, int esq, int dir, int k) {
        ...
        while (i <= j) {
            ...
            if (i <= j) {
                ...
                array[i] = array[j];
                array[j] = temp;
                movimentacoes += 3; // 3 movimentações para o swap
                i++;
                j--;
            }
        }
        ...
        // Chamada recursiva para a direita APENAS se i for menor que k
        if (i < k && i < dir) {
    ...
    public static void main(String[] args) {
            ...
            while (!entrada.equals("FIM") && !entrada.equals("-1")) {
                ...
                for (int i = 0; i < tamanhoColecao; i++) {
                    if (todosRestaurantes[i].getId() == idBuscado) {
                        selecionados[numSelecionados] = todosRestaurantes[i];
                        numSelecionados++;
                        break;
                    }
                }
        ...
        // Chamada do Quicksort Parcial (k = 10)
        quicksortParcial(selecionados, 0, numSelecionados - 1, 10);
        
        long fimTempo = System.currentTimeMillis();
        long tempoExecucao = (fimTempo - inicioTempo); // Tempo em milissegundos
        ...
        try {
            // Arquivo de log com a sua matrícula
            FileWriter log = new FileWriter("855217_quicksort_parcial.txt");
            log.write("855217\t" + comparacoes + "\t" + movimentacoes + "\t" + tempoExecucao + "ms\n");
            log.close();
        } catch (IOException e) {
