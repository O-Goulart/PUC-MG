...
public class Main {
    ...
    public static void selecaoParcial(Restaurante[] array, int n, int k) {
        int limite = Math.min(k, n); // Garante que não vai acessar fora do array se n < k
        
        for (int i = 0; i < limite; i++) {
            int menor = i;
            for (int j = i + 1; j < n; j++) {
                comparacoes++;
                int cmp = array[j].getNome().compareTo(array[menor].getNome());
                
                // Ordenação principal pelo NOME
                if (cmp < 0) {
                    menor = j;
                } 
                // Desempate pelo ID
    ...
    public static void main(String[] args) {
        ...
        // Chamada do Seleção Parcial com k = 10
        selecaoParcial(selecionados, numSelecionados, 10);
        ...
        try {
            FileWriter log = new FileWriter("855217_selecao_parcial.txt");
