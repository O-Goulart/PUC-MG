...
void pesquisar(char* nome) {
    int pos = hash(nome);
    numComparacoes++;
    if (tabela[pos].id != -1 && strcmp(tabela[pos].nome, nome) == 0) {
        printf("%d ", pos);
        imprimir(tabela[pos]);
        return;
    }
    
    for (int i = 31; i < reserva; i++) {
        numComparacoes++;
        if (tabela[i].id != -1 && strcmp(tabela[i].nome, nome) == 0) {
            printf("%d ", i);
            imprimir(tabela[i]);
            return;
        }
...
int main() {
    ...
    FILE* log = fopen("855217_hash_reserva.txt", "w");
    if (log != NULL) {
