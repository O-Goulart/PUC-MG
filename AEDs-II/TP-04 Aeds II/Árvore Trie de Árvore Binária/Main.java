...
public class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            PrintWriter pw = new PrintWriter(new FileWriter("855217_arvore_trie_lista.txt"));
...
class ArvoreTrie {
    ...
    private void inserir(String s, Restaurante r, NoTrie no, int i) {
        NoTrie filho = no.primFilho;
        NoTrie ant = null;

        while (filho != null && filho.elemento != s.charAt(i)) {
            ant = filho;
            filho = filho.irmao;
        }

        if (filho == null) {
            filho = new NoTrie(s.charAt(i));
            if (ant == null) {
                no.primFilho = filho;
            } else {
                ant.irmao = filho;
    ...
    private void pesquisar(String s, NoTrie no, int i) {
        NoTrie filho = no.primFilho;

        while (filho != null) {
            Main.numComparacoes++;
            if (filho.elemento == s.charAt(i)) {
                break;
            }
            filho = filho.irmao;
        }
        ...
        if (filho == null) {
            ...
            if (i == s.length() - 1) {
                if (filho.folha) {
                    System.out.println("SIM " + filho.restaurante.formatar());
                } else {
                    System.out.println("NAO");
                }
            } else {
